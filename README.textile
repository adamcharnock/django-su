 ...
 * SU_LOGIN: The name of a function that returns True if the supplied user can change users

If the SU_LOGIN function takes two arguments, it will be passed a second, which will either be the target user, or None if this is not known at the time of calling (eg from templates showing the button).
